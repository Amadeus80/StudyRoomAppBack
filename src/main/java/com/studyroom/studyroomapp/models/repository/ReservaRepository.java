...
public interface ReservaRepository extends JpaRepository<Reserva, ReservaPK>{
    ...
    @Query("SELECT r FROM Reserva r WHERE r.usuario.id = ?1 and (r.reservaPK.fecha = ?2 and r.reservaPK.horario.id >= ?3) or (r.reservaPK.fecha > ?2) ORDER BY r.reservaPK.fecha ASC, r.reservaPK.horario ASC")
