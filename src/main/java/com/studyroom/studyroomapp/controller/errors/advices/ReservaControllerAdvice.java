...
public class ReservaControllerAdvice {
    ...
    @ResponseBody
    @ExceptionHandler(BorradoReservaException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    ErrorMessage borradoReserva(BorradoReservaException ex){
        return new ErrorMessage(400, ex.getMessage(), "La reserva no pertenece al usuario logeado");
    }
}
